...
/** Photos that exist in /public/food (keeps the seed identical on Vercel and locally). */
const PHOTO_FILES = new Set([
  "adrak-elaichi-cutting-chai", "andhra-chicken-roast-with-parotta", "bbq-chicken-delight-pizza",
  "butter-chicken-roll", "butter-masala-dosa", "butter-tandoori-roti", "cheesy-garlic-bread",
  "chicken-kathi-roll", "chicken-keema-garlic-bread", "chicken-keema-pav",
  "chicken-shawarma-loaded-fries", "chocolate-brownie-with-ice-cream", "coca-cola-can",
  "cold-coffee-with-ice-cream", "creamy-white-sauce-pasta", "crispy-aloo-tikki-burger",
  "crispy-chicken-wings", "crispy-fried-chicken-burger", "dal-makhani-2-butter-kulcha",
  "dal-tadka-with-steamed-rice-bowl", "double-cheese-masala-maggi", "dum-chicken-biryani-bowl",
  "farmhouse-veggie-pizza", "fried-rice-chilli-chicken", "hakka-noodles-chilli-paneer",
  "idli-vada-combo", "loaded-nachos-grande", "monster-energy-drink", "mumbai-egg-bhurji-pav",
  "mysore-masala-dosa", "paneer-butter-masala-2-naan", "paneer-tikka-frankie", "paneer-tikka-pizza",
  "pav-bhaji-platter", "rajma-chawal-bowl", "special-mutton-dum-biryani",
  "spicy-chicken-arrabbiata-pasta", "thums-up-can", "veg-dum-biryani-paneer-65",
]);
...
function photoFor(name: string): string | null {
  const slug = name
    ...
    .replace(/\s+/g, "-")
    .replace(/-+/g, "-");
  return PHOTO_FILES.has(slug) ? `/food/${slug}.jpeg` : null;
}

type Row = { name: string; isVeg: boolean; rupees: number; description: string; prepMinutes: number };
...
export async function runSeed(prisma: PrismaClient): Promise<{ items: number }> {
